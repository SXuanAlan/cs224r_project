#!/usr/bin/env python
"""Train ManiSkill state-observation Flow Matching baselines."""
...
from fgac.data.maniskill_hdf5 import load_maniskill_trajectories, maniskill_dataset_summary
...
def main() -> None:
    ...
    data = load_maniskill_trajectories(
        dataset_path,
        env_id=str(cfg["dataset"]["env_id"]),
        limit_demos=cfg["dataset"].get("limit_demos"),
        obs_key=str(cfg["dataset"].get("obs_key", "obs")),
        action_key=str(cfg["dataset"].get("action_key", "actions")),
    )
    ...
    base_model = TemporalUNetFlow(
        ...
        action_dim=int(train_ds.target_action_dim),
    ...
    metadata = {
        ...
        "dataset": {
            **maniskill_dataset_summary(data),
            ...
            "obs_key": cfg["dataset"].get("obs_key", "obs"),
            "action_key": cfg["dataset"].get("action_key", "actions"),
            "control_mode": cfg["dataset"].get("control_mode", "pd_ee_delta_pose"),
            "reward_mode": cfg["dataset"].get("reward_mode", "sparse"),
        },
        "action": {"source": "maniskill_hdf5", "dim_names": data.dim_names, "groups": data.groups},
    ...
    if cfg.get("simulation_video", {}).get("enabled", False):
        ...
        sim_result = _run_maniskill_eval(ckpt_dir / "best.pt", sim_path, cfg)
...
def _max_steps(cfg: dict[str, Any]) -> int | None:
...
def _build_scheduler(optimizer, cfg, total_steps: int):
    ...
    if sched["type"] != "cosine":
        raise ValueError(f"Unsupported lr_scheduler.type: {sched['type']}")
...
def use_ema_weights(model, ema):
    if ema is None:
        yield
        return
    backup = {name: param.detach().clone() for name, param in model.named_parameters() if param.requires_grad}
    try:
        with torch.no_grad():
            for name, param in model.named_parameters():
                if param.requires_grad:
...
def _run_maniskill_eval(checkpoint_path: Path, output_path: Path, cfg: dict[str, Any]) -> dict[str, Any]:
    sim = cfg.get("simulation_video", {})
    cmd = [
        sys.executable,
        str(PROJECT_ROOT / "scripts" / "eval_maniskill_rollout.py"),
        "--checkpoint",
        str(checkpoint_path.relative_to(PROJECT_ROOT)),
        "--output",
        str(output_path.relative_to(PROJECT_ROOT)),
        "--n-rollouts",
        str(int(sim.get("n_rollouts", 20))),
        "--max-videos",
        str(int(sim.get("max_videos", 0))),
        "--horizon",
        str(int(sim.get("horizon", 200))),
        "--seed",
        str(int(sim.get("seed", cfg["run"].get("seed", 0)))),
    ]
